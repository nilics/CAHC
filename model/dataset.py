...
class BaseDataset(object):
    def __init__(self, type: str, name: str, device: str = 'cpu'):
        ...
        if self.type in ['cocitation', 'coauthorship']:
            self.dataset_dir = osp.join('data','dataset', self.type, self.name) 
        else:
            self.dataset_dir = osp.join('data','dataset', self.name)
...
class yelpRestaurantDataset(BaseDataset):
    def __init__(self, **kwargs):
        super().__init__('cv', 'yelpRestaurant', **kwargs)