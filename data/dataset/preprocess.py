...
def load_hypergcn_dataset(type, name):
    dataset_dir = osp.join('data','dataset','HyperGCN', type, name)
...
def save_preprocessed_dataset(type, name, features, hypergraph, labels):
    if type == 'etc':
        new_dir = osp.join('data','dataset', name)
    else:
        new_dir = osp.join('data','dataset', type, name)
...
def save_masks(type, name, index, masks):
    if type == 'etc':
        new_dir = osp.join('data','dataset', name, 'splits')
    else:
        new_dir = osp.join('data','dataset', type, name, 'splits')
    Path(new_dir).mkdir(parents=True, exist_ok=True)
...
def preprocess_other_dataset(name):
    ...
    p2idx_features_labels = osp.join('data','dataset', name, file_name)
    ...
    p2edges_unordered = osp.join('data','dataset', name, file_name)
...
if __name__ == "__main__":
    ...
    preprocess_other_dataset('yelpRestaurant')